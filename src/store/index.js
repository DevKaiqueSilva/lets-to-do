...
export default new Vuex.Store({
    ...
    listsFiltered(state) {
      return state.lists.filter((l) => (l.show)).map((l) => ({
        ...
        tasks: l.tasks.filter((t) => {
          ...
          if (state.filterIndex == 1) return t.favorite == true;
          if (state.filterIndex == 2) return !!t.planned;
          if (state.filterIndex == 3) return t.completed == false;
  ...
  mutations: {
    ...
    addTask(state, task) {
      const indexL = state.lists.findIndex((l) => (l.id == task.listId));
      ...
      if (indexL >= 0) {
        if (task.id == null) {
          ...
          const indexT = state.lists[indexL].tasks.findIndex((t) => (t.id == task.id));
          console.log(indexT);
    ...
    changeListTitle(state, { id, title }) {
      const indexL = state.lists.findIndex((l) => (l.id == id));
